...
import {
  getDatabase, ref, push, onValue, remove, update, get
...
function startListening() {
  if(!firebaseReady) {
    ...
    cachedPosts = local.map(p => ({key: p.id, data: p}));
  ...
  onValue(postsRef, snapshot => {
    ...
    cachedPosts = Object.entries(val).map(([key, data]) => ({key, data}));
    // Sort ascending by date so newest-first reversal works
    cachedPosts.sort((a,b) => new Date(a.data.date)-new Date(b.data.date));
    renderGrid(cachedPosts);
  });
}
...
async function submitPost() {
  const userName = (document.getElementById('userName')?.value.trim()) || getCurrentUser() || 'Anonymous';
  const caption  = document.getElementById('caption')?.value.trim() || '';

  if(selectedFiles.length===0){ showToast('Please select at least one photo!'); return; }

  const btn = document.getElementById('postBtn');
  btn.disabled = true; btn.textContent = 'Posting…';

  if(userName !== getCurrentUser()) setCurrentUser(userName);
